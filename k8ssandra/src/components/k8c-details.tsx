...
export interface K8ssandraClusterDetailsProps extends Renderer.Component.KubeObjectDetailsProps<K8ssandraCluster>{
}
  ...
  render() {
    ...
    var datacentersStatus = Object.keys(k8c.status.datacenters).map(dc => {
      if (k8c.status.datacenters[dc].cassandra.conditions) {
        return <DrawerItem name={dc} className="status" labelsOnly>
    ...
    var datacentersDetails = k8c.spec.cassandra.datacenters.map(dc => {
      const dcHeader = <div className="drawer-title-module__DrawerTitle--mJBGT drawer-title-module__title--hFfE2">{dc.metadata.name}</div>;
      if (dc.config) {
      ...
      return <div id={dc.metadata.name}>
        {dcHeader}
        <DrawerItem name="Size" labelsOnly>{dc.size}</DrawerItem>
        <DrawerItem name="Stopped" labelsOnly>{dc.stopped}</DrawerItem>
        {dcStorageConfig}<br/><br/>
        {cassandraYaml}
        {jvmOptions}</div>});
...
function displayJvmOptions(jvmOptions: K8ssandraCluster["spec"]["cassandra"]["config"]["jvmOptions"]) {
  ...
  <Table>
    ...
    {Object.keys(jvmOptions).map((key, index) => {
      return (<TableRow key={"jvmOptions" + index}>
        <TableCell>{key}</TableCell>
...
function displayStorageConfig(storageConfig: K8ssandraCluster["spec"]["cassandra"]["storageConfig"]) {
  return <div>
    <DrawerItem name="Storage Class Name" labelsOnly>{storageConfig.cassandraDataVolumeClaimSpec.storageClassName}</DrawerItem>
    <DrawerItem name="Size" labelsOnly>{storageConfig.cassandraDataVolumeClaimSpec.resources.requests.storage}</DrawerItem>
    <DrawerItem name="Access Modes" labelsOnly>{storageConfig.cassandraDataVolumeClaimSpec.accessModes.join(" - ")}</DrawerItem>
    </div>;
}