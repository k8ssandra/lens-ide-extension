...
export class CassandraDatacenterPage extends React.Component<{ extension: Renderer.LensExtension }> {

  render() {
    return (
      <KubeObjectListLayout
                tableId="cassandraDatacentersTable"
                className="CassandraDatacenters" store={cassandraDatacenterStore}
                sortingCallbacks={{
...
function renderProgress(progress: string) {
  ...
  return <Badge
            key={"progress" + progress} label={progress}
            className={className}
          />
}